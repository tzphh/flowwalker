...
class Deepwalk : public WalkerMeta {
 public:
  Deepwalk() {}
  Deepwalk(gpu_graph* _graph, int _max_depth)
...
class PPR : public WalkerMeta {
 ...
 public:
  PPR() {}
  PPR(gpu_graph* _graph, int _max_depth, float _tp)
      : WalkerMeta(_graph, _max_depth) {
    this->tp = _tp;
  }
...
class Node2vec : public WalkerMeta {
 ...
 public:
  Node2vec() {}
  Node2vec(gpu_graph* _graph, int _max_depth, float _p, float _q)
      : WalkerMeta(_graph, _max_depth) {
    this->p = _p;
    this->q = _q;
  }
...
class Metapath : public WalkerMeta {
 ...
 public:
  Metapath() {}
  Metapath(gpu_graph* _graph, int _max_depth, int* _schema, int _schema_len)
      : WalkerMeta(_graph, _max_depth) {
    this->schema = _schema;
  ...
  __device__ weight_t get_weight(Task* task, int i) {
    ...
    return graph->edge_label[offset] == schema[(task->length - 1) % schema_len]
               ? 1.0
               : 0;
  }
  template <typename state_t>
