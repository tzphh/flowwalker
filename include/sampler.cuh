...
template <typename walker_t, typename state_t>
__device__ inline vtx_t sampler_warp(walker_t* walker, Task* task,
                                     state_t* state) {
  vtx_t size = task->degree;
...
template <typename walker_t, typename state_t>
__device__ inline vtx_t sampler_block(walker_t* walker, Task* task,
                                      state_t* state) {
  vtx_t size = task->degree;
  int tid = threadIdx.x;
  ...
  __syncthreads();
  ll selected =
      BlockReduce(temp_storage_reduce).Reduce(selected_id, cub::Max());
  __syncthreads();
...
template <typename walker_t, typename state_t>
__device__ inline vtx_t sampler_warp_oneloop(walker_t* walker, Task* task,
                                             state_t* state) {
  vtx_t size = task->degree;
  int lid = threadIdx.x % WARP_SIZE;
  ...
  for (int i = lid; i < size + WARP_SIZE - size % WARP_SIZE; i += WARP_SIZE) {
    ...
    w = weight_sum;
    if (w > 0 && selected_id == -1) selected_id = 0;
    for (int mask = 1; mask < 32; mask <<= 1) {
      weight_t val =
          __shfl_up_sync(FULL_WARP_MASK, weight_sum, mask, WARP_SIZE);
      if (lid >= mask) weight_sum += val;
    }
    weight_sum += prev_ite_sum;
    if (i < size && myrand_uniform(state) < w / weight_sum) selected_id = i;
...
template <typename walker_t, typename state_t>
__device__ inline vtx_t sampler_block_oneloop(walker_t* walker, Task* task,
                                              state_t* state) {
  vtx_t size = task->degree;
  int tid = threadIdx.x;
  ...
  vtx_t selected_id = -1;
  for (int i = tid; i < size + BLOCK_SIZE - size % BLOCK_SIZE;
       i += BLOCK_SIZE) {
    ...
    // Sample one index.
    if (w > 0 && i < size) {
      if (local_selected_id == -1) local_selected_id = 0;
      if (myrand_uniform(state) < w / weight_sum) local_selected_id = i;
    }
    ...
    selected_id = max(
        selected_id,
        BlockReduce(temp_storage_reduce).Reduce(local_selected_id, cub::Max()));
...
template <typename walker_t, typename state_t>
__device__ inline vtx_t sampler_rjs_warp(walker_t* walker, Task* task,
                                         state_t* state) {
  vtx_t size = task->degree;
  int lid = threadIdx.x % WARP_SIZE;
...
template <typename walker_t, typename state_t>
__device__ inline vtx_t sampler_rjs_block(walker_t* walker, Task* task,
                                          state_t* state) {
  vtx_t size = task->degree;
  int tid = threadIdx.x;
  weight_t local_max_weight = 0;
  ...
  weight_t max_w =
      BlockReduce(temp_storage_reduce).Reduce(local_max_weight, cub::Max());
  __syncthreads();
...
template <typename T>
__device__ inline vtx_t test_thread_sampler(const weight_t* weights, int size,
                                            T* state) {
...
template <typename T>
__device__ inline vtx_t test_warp_sampler(const weight_t* weights, int size,
                                          T* state) {
  int lid = threadIdx.x % WARP_SIZE;
...
template <typename T>
__device__ inline vtx_t test_block_sampler(const weight_t* weights, int size,
                                           T* state) {
  int tid = threadIdx.x;
...
__device__ inline vtx_t test_warp_sampler(const weight_t* weights, int size,
  ...
  if (lid == 0) *clk += clock64() - start_clk;
...
__device__ inline vtx_t test_block_sampler(const weight_t* weights, int size,
  ...
  if (tid == 0) *clk += clock64() - start_clk;

  return selected % size;
}

template <typename T>
__device__ inline vtx_t test_warp_sampler_oneloop(const weight_t* weights,
                                                  int size, T* state) {
  int lid = threadIdx.x % WARP_SIZE;
...
/**
 * N: the number of elements, p: the number of threads, x: global memory access
 * latency Time cost: (N/p) * (x + log p + log p)
 */
template <typename T>
__device__ inline vtx_t test_block_sampler_oneloop_modify(
    const weight_t* weights, int size, T* state) {
  int tid = threadIdx.x;
...
template <typename T>
__device__ inline vtx_t test_block_sampler_oneloop(const weight_t* weights,
                                                   int size, T* state) {
  int tid = threadIdx.x;
...
__device__ inline void construct_table_warp(weight_t* weights, int size,
                                            vtx_t* alias, weight_t* prob,
...
template <typename T>
__device__ inline vtx_t naive_alias_warp_sampler(weight_t* weights, int size,
                                                 T* state, vtx_t* alias,
                                                 weight_t* prob, vtx_t* large,
                                                 vtx_t* small) {
...
__device__ inline void construct_table_block(weight_t* weights, int size,
                                             vtx_t* alias, weight_t* prob,
                                             vtx_t* large, vtx_t* small) {
  int tid = threadIdx.x;
...
template <typename T>
__device__ inline vtx_t naive_alias_block_sampler(weight_t* weights, int size,
                                                  T* state, vtx_t* alias,
                                                  weight_t* prob, vtx_t* large,
                                                  vtx_t* small) {
  // construct alias table
...
template <typename T>
__device__ inline vtx_t its_warp_sampler(weight_t* weights, int size, T* state,
                                         weight_t* prob) {
  int tid = threadIdx.x;
  int lid = tid % WARP_SIZE;
  int wid = tid / WARP_SIZE;
...
template <typename T>
__device__ inline vtx_t its_warp_sampler_direct(weight_t* weights, int size,
                                                T* state, weight_t* prob) {
  int tid = threadIdx.x;
  int lid = tid % WARP_SIZE;
  ...
  for (int i = lid; i < size + WARP_SIZE - size % WARP_SIZE; i += WARP_SIZE) {
    ...
    for (int mask = 1; mask < WARP_SIZE; mask <<= 1) {
      ...
      if (lid >= mask) local_sum += val;
    }
    if (i < size) prob[i] = local_sum;
...
template <typename T>
__device__ inline vtx_t its_block_sampler(weight_t* weights, int size, T* state,
                                          weight_t* prob) {
  int tid = threadIdx.x;
...
template <typename T>
__device__ inline vtx_t its_block_sampler_direct(weight_t* weights, int size,
                                                 T* state, weight_t* prob) {
  int tid = threadIdx.x;
  __shared__ weight_t prefix_sum;
  prefix_sum = 0;
...
template <typename T>
__device__ inline vtx_t rjs_warp_sampler(const weight_t* weights, int size,
                                         T* state, u64* clk) {
  u64 start_clk = clock64();
...
template <typename T>
__device__ inline vtx_t rjs_block_sampler(weight_t* weights, int size,
                                          T* state) {
  int tid = threadIdx.x;
