...
DEFINE_int32(headroom, 512,
             "GPU memory headroom for other data structures while using autobatch(MB)");

DEFINE_int32(walkmode, -1,
             "walkmode:0-static warp,1-static warp block,2-static thread warp "
             "block,3-queue,4-static thread warp,5-dynamic thread warp "
             "block,6-dynamic warp block,7-dynamic warp,8-dynamic block");

DEFINE_bool(lognorm, false, "lognorm edge weight");

void calculate_workload(int num_walkers, int max_depth, vtx_t* result_pool_ptr, graph* ginst,
                        double total_s, bool is_host = false) {
    vtx_t* result = result_pool_ptr;
    if (!is_host) {
        result = reinterpret_cast<vtx_t*>(malloc((u64)sizeof(vtx_t) * num_walkers * max_depth));
    ...
    for (int i = 0; i < num_walkers; i++) {
        ...
        for (int j = 0; j < max_depth; j++) {
            ...
            if (j > 0) sampled_v++;
            if (vtx_pre != -1) sampled_e += ginst->xadj[vtx_pre + 1] - ginst->xadj[vtx_pre];
            vtx_pre = vtx;
        }
    }
...
void degree_bucket(int num_walkers, int max_depth, vtx_t* result_pool_ptr, graph* ginst,
                   bool is_host = false) {
    vtx_t* result = result_pool_ptr;
    if (!is_host) {
        result = reinterpret_cast<vtx_t*>(malloc((u64)sizeof(vtx_t) * num_walkers * max_depth));
...
void print_res(int num_walkers, int max_depth, vtx_t* result_pool, bool is_host = false) {
    ...
    if (!is_host) {
        result = reinterpret_cast<vtx_t*>(malloc((u64)sizeof(vtx_t) * num_walkers * max_depth));
        CUDA_RT_CALL(cudaMemcpy(result, result_pool, (u64)sizeof(vtx_t) * num_walkers * max_depth,
                                cudaMemcpyDeviceToHost));
    }
...
void print_res_metapath(int num_walkers, int max_depth, vtx_t* result_pool, graph* graph,
                        bool is_host = false) {
    vtx_t* result = result_pool;
    if (!is_host) {
        result = reinterpret_cast<vtx_t*>(malloc((u64)sizeof(vtx_t) * num_walkers * max_depth));
...
int main(int argc, char* argv[]) {
    ...
    // {
    ginst =
        new graph(adj_file.c_str(), edge_file.c_str(), weight_file.c_str(), edgelabel_file.c_str());
    // }
    // else
    // {
    //     ginst = new graph(adj_file.c_str(), edge_file.c_str(),
    //     weight_file.c_str(), nullptr);
    // }
    // LOG("Read graph \n");
    printf("Read Graph!\n");
    ...
    if (FLAGS_batch) {
        sample_time = walk_batch(result_pool, ggraph, start_points, depth, sample_size,
                                 FLAGS_batchsize, schema, schema_len);
    } else {
        enum walk_mode mode = wb_dynamic;
        if (FLAGS_walkmode >= 0) {
            mode = walk_mode(FLAGS_walkmode);
        }
        sample_time = walk_test(result_pool, ggraph, start_points, depth, sample_size, mode, schema,
                                schema_len);
    }

    bool is_host = false;
    if (FLAGS_batch) is_host = true;
