...
__global__ void init_taskassignment_batch(TaskAssignments** assign, int num_walkers, int block_num,
                                          int batch_size) {
    int tid = threadIdx.x;
    int bid = blockIdx.x;
...
queue_gpu<Task>* get_queuearr(int max_depth, int num_walkers) {
    ...
    CUDA_RT_CALL(cudaMemcpy(walker_queue_ptr, walker_queue,
                            (u64)max_depth * sizeof(queue_gpu<Task>), cudaMemcpyDefault));
...
double timing(walker_t* walker_ptr, vtx_t* start_points_ptr, vtx_t* result_pool_ptr, int block_num,
              int num_walkers, int max_depth, walk_mode type = wb_dynamic) {
    printf("========start timing\n");
#ifdef MICRO_BENCH
    ...
    if (type == queue) {
        ...
        start_time = wtime();
#ifdef MICRO_BENCH
        walker_queue<<<block_num, BLOCK_SIZE>>>(walker_ptr, walker_queue_ptr, high_queue_ptr,
                                                start_points_ptr, result_pool_ptr, num_walkers,
                                                d_counter);
#else
        walker_queue<<<block_num, BLOCK_SIZE>>>(walker_ptr, walker_queue_ptr, high_queue_ptr,
                                                start_points_ptr, result_pool_ptr, num_walkers);
        ...
        start_time = wtime();
#ifdef MICRO_BENCH
        walker_twb_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, start_pointer,
                                                      result_pool_ptr, num_walkers, d_counter);
#else
        walker_twb_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, start_pointer,
                                                      result_pool_ptr, num_walkers);
#endif
        CUDA_RT_CALL(cudaDeviceSynchronize());
        total_time = wtime() - start_time;
        ...
        if (FLAGS_dprs == true) {
#ifdef MICRO_BENCH
            walker_wb_dynamic_dprs<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
                                                              start_pointer, result_pool_ptr,
                                                              num_walkers, d_counter);
#else
            walker_wb_dynamic_dprs<<<block_num, BLOCK_SIZE>>>(
                walker_ptr, start_points_ptr, start_pointer, result_pool_ptr, num_walkers);
#endif
        } else {
#ifdef MICRO_BENCH
            walker_wb_dynamic_zprs<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr,
                                                              start_pointer, result_pool_ptr,
                                                              num_walkers, d_counter);
#else
            walker_wb_dynamic_zprs<<<block_num, BLOCK_SIZE>>>(
                walker_ptr, start_points_ptr, start_pointer, result_pool_ptr, num_walkers);
#endif
        }
        CUDA_RT_CALL(cudaDeviceSynchronize());
        ...
        start_time = wtime();
#ifdef MICRO_BENCH
        walker_w_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, start_pointer,
                                                    result_pool_ptr, num_walkers, d_counter);
#else
        walker_w_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, start_pointer,
                                                    result_pool_ptr, num_walkers);
        ...
        start_time = wtime();
#ifdef MICRO_BENCH
        walker_b_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, start_pointer,
                                                    result_pool_ptr, num_walkers, d_counter);
#else
        walker_b_dynamic<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, start_pointer,
                                                    result_pool_ptr, num_walkers);
        ...
        if (type == wp) {
            start_time = wtime();
#ifdef MICRO_BENCH
            walker_w<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, result_pool_ptr,
                                                assign_ptr, d_counter);
#else
            walker_w<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, result_pool_ptr,
                                                assign_ptr);
#endif
            ...
            start_time = wtime();
#ifdef MICRO_BENCH
            walker_wb<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, result_pool_ptr,
                                                 assign_ptr, d_counter);
#else
            walker_wb<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, result_pool_ptr,
                                                 assign_ptr);
            ...
            start_time = wtime();
#ifdef MICRO_BENCH
            walker_twb<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, result_pool_ptr,
                                                  assign_ptr, d_counter);
#else
            walker_twb<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, result_pool_ptr,
                                                  assign_ptr);
            ...
            start_time = wtime();
#ifdef MICRO_BENCH
            walker_tw<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, result_pool_ptr,
                                                 assign_ptr, d_counter);
#else
            walker_tw<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, result_pool_ptr,
                                                 assign_ptr);
            ...
            start_time = wtime();
            walker_wb_dprs_curand<<<block_num, BLOCK_SIZE>>>(
                walker_ptr, start_points_ptr, result_pool_ptr, assign_ptr, state_ptr);
            CUDA_RT_CALL(cudaDeviceSynchronize());
            total_time = wtime() - start_time;
        } else if (type == wb_dprs) {
            start_time = wtime();
            walker_wb_dprs<<<block_num, BLOCK_SIZE>>>(walker_ptr, start_points_ptr, result_pool_ptr,
                                                      assign_ptr);
            CUDA_RT_CALL(cudaDeviceSynchronize());
            total_time = wtime() - start_time;
            ...
            CUDA_RT_CALL(cudaMalloc(&state_ptr, block_num * BLOCK_SIZE * sizeof(curandState)));
            start_time = wtime();
            walker_wb_zprs_curand<<<block_num, BLOCK_SIZE>>>(
                walker_ptr, start_points_ptr, result_pool_ptr, assign_ptr, state_ptr);
            CUDA_RT_CALL(cudaDeviceSynchronize());
            total_time = wtime() - start_time;
        }
...
template <typename walker_t>
double timing_batch_async(walker_t* walker_ptr, vtx_t* start_points, vtx_t* result_pool,
    ...
    for (int i = 0; i < num_walkers; i += batch_size * 2) {
        ...
        CUDA_RT_CALL(cudaMemcpyAsync(start_points_ptr, start_points + i,
                                     (u64)sizeof(vtx_t) * batch_num1, cudaMemcpyHostToDevice,
                                     streams[0]));
        CUDA_RT_CALL(cudaMemsetAsync(result_pool_ptr, -1,
                                     (u64)sizeof(vtx_t) * batch_num1 * max_depth, streams[0]));
        CUDA_RT_CALL(cudaMemsetAsync(start_pointer, 0, sizeof(int), streams[0]));
#ifndef TEST_RJS
        if (FLAGS_node2vec) {
            walker_wb_dynamic_dprs<<<block_num, BLOCK_SIZE, 0, streams[0]>>>(
                walker_ptr, start_points_ptr, start_pointer, result_pool_ptr, batch_num1);
        } else {
            walker_wb_dynamic_zprs<<<block_num, BLOCK_SIZE, 0, streams[0]>>>(
                walker_ptr, start_points_ptr, start_pointer, result_pool_ptr, batch_num1);
        }
#else
        walker_wb_dynamic_rjs<<<block_num, BLOCK_SIZE, 0, streams[0]>>>(
            walker_ptr, start_points_ptr, start_pointer, result_pool_ptr, batch_num1);
#endif

        CUDA_RT_CALL(cudaMemcpyAsync(result_pool + (u64)i * max_depth, result_pool_ptr,
        ...
        if (batch_num2 > 0) {
            ...
            CUDA_RT_CALL(cudaMemcpyAsync(b_start_points_ptr, start_points + j,
                                         (u64)sizeof(vtx_t) * batch_num2, cudaMemcpyHostToDevice,
                                         streams[1]));
            CUDA_RT_CALL(cudaMemsetAsync(b_result_pool_ptr, -1,
                                         (u64)sizeof(vtx_t) * batch_num2 * max_depth, streams[1]));
            CUDA_RT_CALL(cudaMemsetAsync(start_pointer + 1, 0, sizeof(int), streams[1]));
#ifndef TEST_RJS
            if (FLAGS_node2vec) {
                walker_wb_dynamic_dprs<<<block_num, BLOCK_SIZE, 0, streams[1]>>>(
                    walker_ptr, b_start_points_ptr, start_pointer + 1, b_result_pool_ptr,
                    batch_num2);
            } else {
                walker_wb_dynamic_zprs<<<block_num, BLOCK_SIZE, 0, streams[1]>>>(
                    walker_ptr, b_start_points_ptr, start_pointer + 1, b_result_pool_ptr,
                    batch_num2);
            }
#else
            walker_wb_dynamic_rjs<<<block_num, BLOCK_SIZE, 0, streams[1]>>>(
                walker_ptr, b_start_points_ptr, start_pointer + 1, b_result_pool_ptr, batch_num2);
#endif
            CUDA_RT_CALL(cudaMemcpyAsync(result_pool + (u64)j * max_depth, b_result_pool_ptr,
...
template <typename walker_t>
double timing_batch_sync(walker_t* walker_ptr, vtx_t* start_points, vtx_t* result_pool,
                         int batch_size, int num_walkers, int max_depth, int block_num) {
    int stream_num =
        (num_walkers % batch_size == 0) ? num_walkers / batch_size : num_walkers / batch_size + 1;
    ...
    for (int i = 0; i < stream_num; i++) {
        ...
        CUDA_RT_CALL(cudaMemcpy(start_points_ptr, start_points + (u64)i * batch_size,
        ...
        if (FLAGS_node2vec) {
            walker_wb_dynamic_dprs<<<block_num, BLOCK_SIZE>>>(
                walker_ptr, start_points_ptr, start_pointer, result_pool_ptr, batch_num);
        } else {
            walker_wb_dynamic_zprs<<<block_num, BLOCK_SIZE>>>(
                walker_ptr, start_points_ptr, start_pointer, result_pool_ptr, batch_num);
        }
        ...
        transfer_s = wtime();
        CUDA_RT_CALL(cudaMemcpy(result_pool + (u64)i * batch_size * max_depth, result_pool_ptr,
...
double walk_test(vtx_t*& result_pool_ptr, gpu_graph* graph,
                 ...
                 int max_depth, int num_walkers, walk_mode type, int* schema, int schema_len) {
    LOG("%s\n", __FUNCTION__);
    double total_time;
    ...
    if (FLAGS_deepwalk) {
        ...
        total_time = timing<Deepwalk>(walker_ptr, start_points_ptr, result_pool_ptr, block_num,
                                      num_walkers, max_depth, type);
    } else if (FLAGS_ppr) {
        PPR* walker = new PPR(graph_ptr, max_depth, FLAGS_tp);
        PPR* walker_ptr = get_device_ptr<PPR>(walker, 1);
        total_time = timing<PPR>(walker_ptr, start_points_ptr, result_pool_ptr, block_num,
                                 num_walkers, max_depth, type);
        ...
        total_time = timing<Node2vec>(walker_ptr, start_points_ptr, result_pool_ptr, block_num,
                                      num_walkers, max_depth, type);
        ...
        total_time = timing<Metapath>(walker_ptr, start_points_ptr, result_pool_ptr, block_num,
                                      num_walkers, max_depth, type);
    } else {
...
double walk_batch(vtx_t*& result_pool, gpu_graph* graph,
                  vtx_t* start_points,  // NOLINT
                  int max_depth, int num_walkers, int batch_size, int* schema, int schema_len) {
    LOG("%s\n", __FUNCTION__);
    double total_time;
    ...
    if (FLAGS_deepwalk) {
        ...
        if (FLAGS_syn)
            total_time = timing_batch_sync(walker_ptr, start_points, result_pool, batch_size,
                                           num_walkers, max_depth, block_num);
        else
            total_time = timing_batch_async(walker_ptr, start_points, result_pool, batch_size,
                                            num_walkers, max_depth, block_num);
    } else if (FLAGS_ppr) {
        PPR* walker = new PPR(graph_ptr, max_depth, FLAGS_tp);
        PPR* walker_ptr = get_device_ptr<PPR>(walker, 1);
